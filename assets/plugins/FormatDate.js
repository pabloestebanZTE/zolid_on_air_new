...
// WWW: http://www.mattkruse.com/javascript/date/source.html
//
 ...
 * @type String
 */
function formatDate(date, format, formatDate) {
    if (typeof date === "string") {
        if (typeof formatDate != "string") {
            formatDate = "dd/MM/yyyy";
        }
        var dateStr = date;
        var complmt = date.slice(10);
        var parts = dateStr.match(/(\d+)/g);
        var partsFormat = formatDate.match(/[a-z|A-Z]+/g);
        var day, month, year;
        for (var i = 0; i < partsFormat.length; i++) {
            if (partsFormat[i] === "dd") {
                day = parts[i];
            } else if (partsFormat[i] === "MM") {
                month = parts[i];
            } else if (partsFormat[i] === "yyyy") {
                year = parts[i];
            }
        }
